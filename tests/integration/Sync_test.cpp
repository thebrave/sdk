...
struct StandardClient : public MegaApp
{
    ...
    bool recursiveConfirm(Model::ModelNode* mn, LocalNode* n, int& descendants, const string& identifier, int depth, bool& firstreported)
    {
        ...
        for (auto& n2 : n->children)
        {
            ...
            if (!(child.deleted || child.excluded()))
            {
                ...
                ns.emplace(n2.second->name, n2.second);
            }
        }
    ...
    {
        resultproc.processresult(SETATTR, e, UNDEF);
    }
    ...
    void putnodes(handle parentHandle, vector<NewNode>&& nodes, std::promise<bool>& result)
    {
    ...
    bool putnodes(handle parentHandle, vector<NewNode>&& nodes)
    {
        auto result =
         thread_do([&](StandardClient& client, promise<bool>& result)
                    {
    ...
    void putnodes_prepareOneFolder(NewNode& node, string name)
    {
        return client.putnodes_prepareOneFolder(&node, std::move(name));
    }
    ...
    bool deleteremote(string path)
    {
        auto result =
          thread_do([&](StandardClient& client, promise<bool>& result)
                    {
                        client.deleteremote(std::move(path), result);
                    });
    ...
    {
        Node* debris = drillchildnodebyname(getcloudrubbishnode(), "SyncDebris");
    ...
    void deleteremotenode(Node* node, promise<bool>& result)
    {
        resultproc.prepresult(UNLINK,
                              ++next_request_tag,
                              [&]()
                              {
                                  client.unlink(node, false, client.reqtag);
                              },
                              [&](error e)
                              {
                                  result.set_value(!e);
...
TEST_F(FilterFixture, OrderDependentFilter)
{
    ...
    // a, abc are excluded from the local node tree.
    localTree = localFS;
    localTree.removenode("a");
    localTree.removenode("abc");
