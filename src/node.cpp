...
bool LocalNode::excluded(const string& name, const nodetype_t type) const
{
    ...
    // Search for a filter match.
    for (auto* node = this; node; node = node->parent)
    {
        const auto result =
          node->mFilters.match(namePath, type, node != this);
    ...
    // Compute local name.
    const auto localName = LocalPath::fromName(name, fsAccess, fsType);
